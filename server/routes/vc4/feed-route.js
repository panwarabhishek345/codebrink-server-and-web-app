...
router.get("/feed", (req, res) => {
  ...
  if (topic != null && subtopic != null) {
    ...
    if (reqNext != null) {
      ArticleObject.find({ createdAt: { $lt: reqNext }, status: "published" })
        .select("-mediaObjects")
        .populate("author")
        .sort({ createdAt: -1 })
        .limit(limit)
      ...
      ArticleObject.find({ createdAt: { $gt: reqPrev }, status: "published" })
        .select("-mediaObjects")
        .populate("author")
        .sort({ createdAt: -1 })
        .limit(limit)
      ...
      ArticleObject.find({ status: "published" })
        .select("-mediaObjects")
        .populate("author")
        .sort({ createdAt: -1 })
        .limit(limit)
...
router.get("/googleUser/feed", authenticateGoogle, (req, res) => {
  var requestPage = req.header("page");
  var requestLimit = req.header("limit");
  var reqNext = req.header("reqNext");
  var reqPrev = req.header("reqPrev");
